...
class Member_profile_model extends MY_Model
{
	...
	public function read($start_row, $per_page)
	{
		...
		if ($order_by == '') {
			$order_by	= " $this->my_table.userid DESC"; //Default
		}
		...
		$this->db->select("$this->my_table.*, tb_department_1.dpm_name AS departmentIdDepartmentName
				, tb_members_2.firstname AS createUserIdFullname, tb_members_2.lastname AS createUserIdLastname
				, _doc_tb_members_3.firstname AS modifyUserIdFullname, _doc_tb_members_3.lastname AS modifyUserIdLastname
				");
		...
		if ($query->num_rows() > 0) {
			...
			if ($member_id == NULL) { //from member edit
				...
				if ($this->isAdmin() == TRUE) {
					$admin_password = '';
					//Get admin pass
					$this->db->where(array('userid' => $this->session->userdata('user_id')));
					if ($query = $this->db->get($this->my_table)) {
						if ($row = $query->row()) {
					...
					// Verify encrypt pass
					$admin_encrypt = $this->Login_model->encrypt_md5_salt($data['adminPassword']);
					if (password_verify($admin_encrypt, $admin_password)) {
						$check_pass = TRUE;
					} else {
						$this->error_message = "รหัสผ่านแอดมินผู้แก้ไขไม่ถูกต้อง กรุณาตรวจสอบรหัสผ่านของ แอดมิน อีกครั้ง";
						return;
					}
				}
			}
			...
			if ($check_pass == TRUE) {
				...
				$data = array(
					'password' 		=> $new_pass, 'modify_datetime' 	=> date('Y-m-d H:i:s'), 'modify_user_id' 	=> $userid
				);
				$this->set_where("userid = $userid");
