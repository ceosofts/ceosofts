...
class Members extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->data['page_url'] = site_url('member/members');
		...
		$js_url = 'assets/js_modules/member/members.js?ft=' . filemtime('assets/js_modules/member/members.js');
	...
	protected function render_view($path)
	{
		...
		if ($this->session->userdata('login_validated') == false) {
			...
			if ($this->session->userdata('user_level') == 1 && $this->session->userdata('user_department_id') == 12) {
	...
	public function search()
	{
		...
		$page_url = site_url('member/members');
		...
		$this->render_view('member/members/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'Members', 'url' => site_url('member/members')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('member/members/preview_view');
			}
		}
	}
	...
	{
		// load PDF library
		$this->load->library('member/Members_preview_pdf');
		...
		$html = $this->parser->parse_repeat('member/members/preview_view_pdf', $data, true);
	...
	public function preview_export_excel($encrypt_id = "")
	{
		...
		$table	=  $this->parser->parse_repeat('member/members/preview_view_excel', $data, true);
	...
	public function add()
	{
		...
		$this->setFormAddData();
		$this->render_view('member/members/add_view');
	}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->data['tb_department_department_id_option_list'] = $this->Members->returnOptionList("tb_department", "dpm_id", "dpm_name");
				$this->data['tb_status_void_option_list'] = $this->Members->returnOptionList("tb_status", "dpm_void", "dpm_name");
				$this->render_view('member/members/edit_view');
			}
		}
	}
	...
	{
		// load excel library
		$this->load->library('member/Members_list_pdf');
		...
		$html = $this->parser->parse_repeat('member/members/list_view_pdf', $data, true);
		$pdf->writeHTMLCell(0, 0, '', '', $html, 0, 1, 0, true, '', true);
	...
	{
		// load excel library
		$this->load->library('member/Excel');
