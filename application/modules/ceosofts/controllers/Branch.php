 ...
 */
class Branch extends MEMBER_Controller
{
	...
	public function __construct()
	{
		...
		$this->num_links = 6;
		$this->uri_segment = 4;
		$this->load->model('ceosofts/Branch_model', 'Branch');
		$this->Branch->session_name = 'ceosofts_branch';

		$this->data['page_url'] = site_url('ceosofts/branch');
		...
		$js_url = 'assets/js_modules/ceosofts/branch.js?ft=' . filemtime('assets/js_modules/ceosofts/branch.js');
	...
	public function search()
	{
		...
		$page_url = site_url('ceosofts/branch');
		...
		$this->render_view('ceosofts/branch/list_view');
	}
	...
	{
		$this->breadcrumb_data['breadcrumb'] = array(
			array('title' => 'Branch', 'url' => site_url('ceosofts/branch')),
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->render_view('ceosofts/branch/preview_view');
			}
		}
	}
	...
	public function reload_runninng()
	{
		...
		echo $this->Branch->set_running_number($field);
	}
	...
	{
		$branch_id_running = $this->Branch->set_running_number('branch_id');
		$this->data['source_branch_id'] = $branch_id_running;
	}
	...
	public function add()
	{
		...
		$this->setFormAddData();
		$this->render_view('ceosofts/branch/add_view');
	}
	...
	public function formValidate()
	{
		...
		$frm->set_rules('branch_id', 'หมายเลขข้อมูล', 'trim|required');
		$frm->set_rules('branch_name', 'ชื่อสาขา', 'trim|required');
	...
	public function save()
	{
		...
		if ($message != '') {
			...
			if ($id != '') {
				...
				$success = FALSE;
				$message = 'Error : ' . $this->Branch->error_message;
			}
	...
	public function edit($encrypt_id = '')
	{
		...
		if ($id == "") {
			...
			if (empty($results)) {
				...
				$this->data['source_branch_id'] = '';

				$this->setPreviewFormat($results);

				$this->render_view('ceosofts/branch/edit_view');
			}
		}
	}
