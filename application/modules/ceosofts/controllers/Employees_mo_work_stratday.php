 ...
 */
class Employees extends CRUD_Controller
{
	...
	public function add()
	{
		...
		$this->data['tb_members_prefix_emp_title_name_option_list'] = $this->Employees->returnOptionList("tb_members_prefix", "id", "prefix");
		$this->data['tb_position_emp_position_option_list'] = $this->Employees->returnOptionList("tb_position", "id", "position_name");
		$this->data['tb_department_emp_section_option_list'] = $this->Employees->returnOptionList("tb_department", "dpm_id", "dpm_name");
	...
	public function formValidate()
	{
		...
		$frm->set_rules('emd_id_card', 'เลขบัตรประชาชน', 'trim|required|is_natural');
		//file upload
		$check_file = FALSE;
		if ($this->input->post('emp_photo_label') == '') {
			$check_file = TRUE;
		}
		if ($check_file == TRUE) {
			if (empty($_FILES['emp_photo']['name'])) {
				$frm->set_rules('emp_photo', 'รูปพนักงาน', 'trim|required');
			}
		}
		$frm->set_rules('emp_sex', 'เพศ[1=ชาย, 2=หญิง]', 'trim|required');
		$frm->set_rules('emp_birthday', 'วันที่เกิด', 'trim|required');
		$frm->set_rules('emp_age', 'อายุ', 'trim|required|is_natural');
		...
		$frm->set_rules('emp_time', 'อายุงาน', 'trim|required|decimal');
		$frm->set_rules('emp_end', 'วันที่ลาออก', 'trim|required');
		$frm->set_rules('emp_holiday_max', 'จำนวนวันลาพักร้อน', 'trim|required|is_natural');
		$frm->set_rules('emp_holiday_off', 'จำนวนวันที่ลาพักร้อนแล้ว', 'trim|required|is_natural');
		$frm->set_rules('emp_dayoff_day', 'จำนวนวันหยุด', 'trim|required|is_natural');
		$frm->set_rules('emp_dayoff_off', 'จำนวนวันที่หยุดแล้ว', 'trim|required|is_natural');
		...
		if ($frm->run() == FALSE) {
			...
			$message .= form_error('emp_time');
			$message .= form_error('emp_end');
			$message .= form_error('emp_holiday_max');
			$message .= form_error('emp_holiday_off');
			$message .= form_error('emp_dayoff_day');
			$message .= form_error('emp_dayoff_off');
			return $message;
		}
	}
	...
	private function setPreviewFormat($row_data)
	{
		...
		$empTitleNamePrefixName = $this->table('tb_members_prefix')->get_value('prefix')->where("id = '$data[emp_title_name]'");
		$this->data['empTitleNamePrefixName'] = $empTitleNamePrefixName;


		$empPositionPositionName = $this->table('tb_position')->get_value('position_name')->where("id = '$data[emp_position]'");
		$this->data['empPositionPositionName'] = $empPositionPositionName;


		$empSectionDpmName = $this->table('tb_department')->get_value('dpm_name')->where("dpm_id = '$data[emp_section]'");
	...
	{
		$emp_start = $this->input->post('emp_start', TRUE);
		if ($emp_start) {
			$emp_start = setDateToStandard($emp_start);

			$emp_end = $this->input->post('emp_end', TRUE);

			if (!$emp_end) {
				$emp_end = date('Y-m-d');
			} else {
				$emp_end = setDateToStandard($emp_end);
			}


			$arr = ci_date_diff($emp_start, $emp_end);
