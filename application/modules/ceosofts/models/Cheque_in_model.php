...
/**
 * Cheque_in_model Class
 * @date 2021-10-08
 */
class Cheque_in_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['chequein_id'] = array('format' => 1, 'type' => 'CIN', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'chequein_id' => $post['chequein_id'], 'chequein_make_item' => setDateToStandard($post['chequein_make_item']), 'chequein_receive_date' => setDateToStandard($post['chequein_receive_date']), 'chequein_plan_date' => setDateToStandard($post['chequein_plan_date']), 'chequein_from' => $post['chequein_from'], 'chequein_price' => str_replace(",", "", $post['chequein_price']), 'chequein_bookbank' => $post['chequein_bookbank'], 'chequein_number' => $post['chequein_number'], 'chequein_status' => $post['chequein_status'], 'chequein_photo' => $post['chequein_photo'], 'chequein_remark' => $post['chequein_remark'], 'chequein_user_make' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_customer AS tb_customer_1', "$this->my_table.chequein_from = tb_customer_1.cus_name", 'left');
			$this->db->join('tb_bookbank AS tb_bookbank_2', "$this->my_table.chequein_bookbank = tb_bookbank_2.bank_cus_name", 'left');
			$this->db->join('tb_cheque_status AS tb_cheque_status_3', "$this->my_table.chequein_status = tb_cheque_status_3.chs_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_customer_1.cus_name AS chequeinFromCusName
				, tb_bookbank_2.bank_cus_name AS chequeinBookbankBankCusName, tb_bookbank_2.bank_name AS chequeinBookbankBankName, tb_bookbank_2.bank_branch AS chequeinBookbankBankBranch, tb_bookbank_2.bank_number AS chequeinBookbankBankNumber
				, tb_cheque_status_3.chs_name AS chequeinStatusChsName
				");
		$this->db->join('tb_customer AS tb_customer_1', "$this->my_table.chequein_from = tb_customer_1.cus_name", 'left');
		$this->db->join('tb_bookbank AS tb_bookbank_2', "$this->my_table.chequein_bookbank = tb_bookbank_2.bank_cus_name", 'left');
		$this->db->join('tb_cheque_status AS tb_cheque_status_3', "$this->my_table.chequein_status = tb_cheque_status_3.chs_name", 'left');
	...
	{
		$data = array(
			'chequein_id' => $post['chequein_id'], 'chequein_make_item' => setDateToStandard($post['chequein_make_item']), 'chequein_receive_date' => setDateToStandard($post['chequein_receive_date']), 'chequein_plan_date' => setDateToStandard($post['chequein_plan_date']), 'chequein_from' => $post['chequein_from'], 'chequein_price' => str_replace(",", "", $post['chequein_price']), 'chequein_bookbank' => $post['chequein_bookbank'], 'chequein_number' => $post['chequein_number'], 'chequein_status' => $post['chequein_status'], 'chequein_remark' => $post['chequein_remark'], 'chequein_user_make' => $this->session->userdata('user_firstname')
		);

		if (isset($post['chequein_photo'])) {
			$data['chequein_photo'] = $post['chequein_photo'];
		}
