...
/**
 * Stock_balance_model Class
 * @date 2021-10-08
 */
class Stock_balance_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_stock_balance';
		...
		$options = array();
		$options['stb_id'] = array('format' => 1, 'type' => 'STB', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				'stb_id' => $post['stb_id']
				,'stb_date' => setDateToStandard($post['stb_date'])
				,'stb_remark' => $post['stb_remark']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	...
	{
		$this->set_table_name('tb_stock_balance_list');
		$this->db->select("tb_stock_balance_list.*, tb_product_buy_1.prb_name AS detailStbProductNamePrbName, tb_stock_balance_status_2.sto_stastus_name AS detailStbStatusStoStastusName");
		$this->db->join('tb_product_buy AS tb_product_buy_1', "tb_stock_balance_list.stb_product_name = tb_product_buy_1.prb_name", 'left');
		$this->db->join('tb_stock_balance_status AS tb_stock_balance_status_2', "tb_stock_balance_list.stb_status = tb_stock_balance_status_2.sto_stastus_name", 'left');
		$this->set_where("stb_pr_ref = '$master_ref_id'");
	...
	public function save_detail_list($post)
	{
		$this->set_table_name('tb_stock_balance_list');
		$data = array(
				'stb_pr_ref' => $post['stb_pr_ref']
				,'stb_no' => $post['stb_no']
				,'stb_product_name' => $post['stb_product_name']
				,'stb_unit' => $post['stb_unit']
				,'stb_qty' => $post['stb_qty']
				,'stb_status' => $post['stb_status']
				,'stb_remark' => $post['stb_remark']
				,'stb_edit_date' => setDateToStandard($post['stb_edit_date'])
		);
