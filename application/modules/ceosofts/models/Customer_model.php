...
class Customer_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['cus_id'] = array('format' => 1, 'type' => 'CUS', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			...
			'cus_contact' => $post['cus_contact'],
			'cus_address' => $post['cus_address'],
			'cus_tel' => $post['cus_tel'],
			'cus_tax' => $post['cus_tax'],
			'cus_branch' => $post['cus_branch']
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.cus_branch = tb_branch_1.branch_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_branch_1.branch_name AS cusBranchBranchName
				");
		$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.cus_branch = tb_branch_1.branch_name", 'left');
