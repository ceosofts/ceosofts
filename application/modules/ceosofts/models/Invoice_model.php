...
class Invoice_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['inv_id'] = array('format' => 1, 'type' => 'INV', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'inv_id' => $post['inv_id'],
			'inv_quo_number_ref' => $post['inv_quo_number_ref'],
			'inv_date' => setDateToStandard($post['inv_date']),
			'inv_cus' => $post['inv_cus'],
			'inv_project_name' => $post['inv_project_name'],
			'inv_price' => str_replace(",", "", $post['inv_price']),
			'inv_price_this_period' => str_replace(",", "", $post['inv_price_this_period']),
			'inv_status' => $post['inv_status'],
			'inv_pay_date' => setDateToStandard($post['inv_pay_date']),
			'inv_by' => $this->session->userdata('user_firstname'),
			'inv_create_date' => date('Y-m-d')
		);
		$this->set_table_name($this->my_table);
	...
	{
		$this->db->insert_batch('tb_invoice_quotation_list', $array_data);
		return $this->db->affected_rows();
	}
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_quotation AS tb_quotation_1', "$this->my_table.inv_quo_number_ref = tb_quotation_1.quo_id", 'left');
			$this->db->join('tb_customer AS tb_customer_2', "$this->my_table.inv_cus = tb_customer_2.cus_name", 'left');
			$this->db->join('tb_invoice_status AS tb_invoice_status_3', "$this->my_table.inv_status = tb_invoice_status_3.sti_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_quotation_1.quo_id AS invQuoNumberRefQuoId
				, tb_customer_2.cus_name AS invCusCusName, tb_customer_2.cus_contact AS invCusCusContact, tb_customer_2.cus_address AS invCusCusAddress, tb_customer_2.cus_tel AS invCusCusTel, tb_customer_2.cus_tax AS invCusCusTax, tb_customer_2.cus_branch AS invCusCusBranch
				, tb_invoice_status_3.sti_name AS invStatusStiName
				");
		$this->db->join('tb_quotation AS tb_quotation_1', "$this->my_table.inv_quo_number_ref = tb_quotation_1.quo_id", 'left');
		$this->db->join('tb_customer AS tb_customer_2', "$this->my_table.inv_cus = tb_customer_2.cus_name", 'left');
		$this->db->join('tb_invoice_status AS tb_invoice_status_3', "$this->my_table.inv_status = tb_invoice_status_3.sti_name", 'left');
	...
	public function update($post)
	{
		$data = array(
			...
			'inv_status' => $post['inv_status'],
			'inv_pay_date' => setDateToStandard($post['inv_pay_date']),
			'inv_edit_by' => $this->session->userdata('user_firstname'),
			'inv_edit_date' => date('Y-m-d')
		);
	...
	{
		$this->set_table_name('tb_invoice_quotation_list');
		$this->db->select("tb_invoice_quotation_list.*, tb_unit_1.unit_name AS detailInvProUnitUnitName");
		$this->db->join('tb_unit AS tb_unit_1', "tb_invoice_quotation_list.inv_pro_unit = tb_unit_1.unit_name", 'left');
		$this->set_where("inv_ref = '$master_ref_id'");
	...
	public function save_detail_list($post)
	{
		$this->set_table_name('tb_invoice_quotation_list');
		$data = array(
			'inv_ref' => $post['inv_ref'],
			'inv_pro_name' => $post['inv_pro_name'],
			'inv_pro_price' => str_replace(",", "", $post['inv_pro_price']),
			'inv_pro_unit' => $post['inv_pro_unit'],
			'inv_pro_qty' => str_replace(",", "", $post['inv_pro_qty']),
			'inv_pro_remark' => $post['inv_pro_remark']
		);
