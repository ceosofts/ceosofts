...
class Payment_receipt_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_payment_receipt';
		...
		$options = array();
		$options['pay_rec_id'] = array('format' => 1, 'type' => 'PAM', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'pay_rec_id' => $post['pay_rec_id'],
			'pay_rec_pr_ref' => $post['pay_rec_pr_ref'],
			'pay_rec_date' => setDateToStandard($post['pay_rec_date']),
			'pay_rec_sup' => $post['pay_rec_sup'],
			'pay_rec_project_name' => $post['pay_rec_project_name'],
			'pay_rec_price' => str_replace(",", "", $post['pay_rec_price']),
			'pay_rec_pay_by' => $post['pay_rec_pay_by'],
			'pay_rec_remark' => $post['pay_rec_remark'],
			'pay_rec_scan' => $post['pay_rec_scan'],
			'pay_rec_pay_date' => setDateToStandard($post['pay_rec_pay_date']), //วันที่ได้รับเอกสาร
			'pay_rec_by' => $this->session->userdata('user_firstname') //ผู้จัดทำเอกสาร
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_pay AS tb_pay_1', "$this->my_table.pay_rec_pr_ref = tb_pay_1.pay_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_pay_1.pay_id AS payRecPrRefPayId
				");
		$this->db->join('tb_pay AS tb_pay_1', "$this->my_table.pay_rec_pr_ref = tb_pay_1.pay_id", 'left');
	...
	public function update($post)
	{
		$data = array(
			...
			'pay_rec_pay_by' => $post['pay_rec_pay_by'],
			'pay_rec_remark' => $post['pay_rec_remark'],
			'pay_rec_pay_date' => setDateToStandard($post['pay_rec_pay_date']),
			'pay_rec_by' => $post['pay_rec_by'],
			'pay_rec_edit_by' => $this->session->userdata('user_firstname'),
			'pay_rec_edit_date' => date('Y-m-d')
		);

		if (isset($post['pay_rec_scan'])) {
			$data['pay_rec_scan'] = $post['pay_rec_scan'];
		}
