...
/**
 * cheque_status_model Class
 * @date 2021-10-03
 */
class cheque_status_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_cheque_status';
	...
	{
		$chs_id = checkEncryptData($data['chs_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.chs_id = $chs_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.chs_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'chs_name' => $post['chs_name'],
			'chs_void' => $post['chs_void']
		);
		$this->set_table_name($this->my_table);
	...
	{
		$data = array(
			'chs_name' => $post['chs_name'], 'chs_void' => $post['chs_void']
		);

		if (!empty($data)) {
			$chs_id = checkEncryptData($post['encrypt_chs_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.chs_id = $chs_id");
