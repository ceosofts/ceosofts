...
/**
 * Employees_salary_slip_model Class
 * @date 2021-10-10
 */
class Employees_salary_slip_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_employees_salary_slip';
		...
		$options = array();
		$options['Slip_id'] = array('format' => 1, 'type' => 'SLP', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				...
				,'slip_ems_id' => $post['slip_ems_id']
				,'slip_ems_fname' => $post['slip_ems_fname']
				,'slip_ems_lname' => $post['slip_ems_lname']
				,'slip_ems_salary' => str_replace(",", "", $post['slip_ems_salary'])
				,'slip_ems_ot' => str_replace(",", "", $post['slip_ems_ot'])
				,'slip_ems_advance' => str_replace(",", "", $post['slip_ems_advance'])
				,'slip_ems_ss' => str_replace(",", "", $post['slip_ems_ss'])
				,'slip_ems_absent' => str_replace(",", "", $post['slip_ems_absent'])
				,'slip_tax' => str_replace(",", "", $post['slip_tax'])
				,'slip_net' => str_replace(",", "", $post['slip_net'])
				,'slip_ems_remark' => $post['slip_ems_remark']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_employees_salary AS tb_employees_salary_1', "$this->my_table.slip_ems_id = tb_employees_salary_1.ems_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_employees_salary_1.ems_id AS slipEmsIdEmsId
				");
		$this->db->join('tb_employees_salary AS tb_employees_salary_1', "$this->my_table.slip_ems_id = tb_employees_salary_1.ems_id", 'left');
