...
class Bank_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['bank_id'] = array('format' => 1, 'type' => 'BAK', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'bank_id' => $post['bank_id'], 'bankout_date' => setDateToStandard($post['bankout_date']), 'bank_cus_name' => $post['bank_cus_name'], 'bank_balance_before' => str_replace(",", "", $post['bank_balance_before']), 'bank_action' => $post['bank_action'], 'bank_price' => str_replace(",", "", $post['bank_price']), 'bank_balance_after' => str_replace(",", "", $post['bank_balance_after']), 'bank_remark' => $post['bank_remark'], 'bank_edit_by' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.bank_cus_name = tb_bookbank_1.bank_cus_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_bookbank_1.bank_cus_name AS bankCusNameBankCusName, tb_bookbank_1.bank_name AS bankCusNameBankName, tb_bookbank_1.bank_branch AS bankCusNameBankBranch, tb_bookbank_1.bank_number AS bankCusNameBankNumber
				");
		$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.bank_cus_name = tb_bookbank_1.bank_cus_name", 'left');
