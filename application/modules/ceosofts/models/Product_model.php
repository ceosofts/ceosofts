...
class Product_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['pro_id'] = array('format' => 1, 'type' => 'PRO', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				...
				,'pro_price' => str_replace(",", "", $post['pro_price'])
				,'pro_unit' => $post['pro_unit']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_unit AS tb_unit_1', "$this->my_table.pro_unit = tb_unit_1.unit_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_unit_1.unit_name AS proUnitUnitName
				");
		$this->db->join('tb_unit AS tb_unit_1', "$this->my_table.pro_unit = tb_unit_1.unit_name", 'left');
