 ...
 * @date 2021-10-08
 */
class Bank_in_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['bank_in_id'] = array('format' => 1, 'type' => 'BAI', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'bank_in_id' => $post['bank_in_id'], 'bank_in_date' => setDateToStandard($post['bank_in_date']), 'bank_in_name' => $post['bank_in_name'], 'bank_in_balance_before' => str_replace(",", "", $post['bank_in_balance_before']), 'bank_in_price' => str_replace(",", "", $post['bank_in_price']), 'bank_in_balance_after' => str_replace(",", "", $post['bank_in_balance_after']), 'bank_in_remark' => $post['bank_in_remark'], 'bank_in_edit_by' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.bank_in_name = tb_bookbank_1.bank_cus_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_bookbank_1.bank_cus_name AS bankInNameBankCusName, tb_bookbank_1.bank_name AS bankInNameBankName, tb_bookbank_1.bank_branch AS bankInNameBankBranch, tb_bookbank_1.bank_number AS bankInNameBankNumber
				");
		$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.bank_in_name = tb_bookbank_1.bank_cus_name", 'left');
	...
	{
		$data = array(
			'bank_in_id' => $post['bank_in_id'],
			'bank_in_date' => setDateToStandard($post['bank_in_date']),
			'bank_in_name' => $post['bank_in_name'],
			'bank_in_balance_before' => str_replace(",", "", $post['bank_in_balance_before']),
			'bank_in_price' => str_replace(",", "", $post['bank_in_price']),
			'bank_in_balance_after' => str_replace(",", "", $post['bank_in_balance_after']),
			'bank_in_remark' => $post['bank_in_remark'],
			'bank_in_edit_by' => $this->session->userdata('user_firstname')
		);
