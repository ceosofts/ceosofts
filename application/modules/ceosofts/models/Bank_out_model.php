...
/**
 * Bank_out_model Class
 * @date 2021-10-08
 */
class Bank_out_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['bank_out_id'] = array('format' => 1, 'type' => 'BAO', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'bank_out_id' => $post['bank_out_id'], 'bank_out_date' => setDateToStandard($post['bank_out_date']), 'bank_out_name' => $post['bank_out_name'], 'bank_out_balance_before' => str_replace(",", "", $post['bank_out_balance_before']), 'bank_out_price' => str_replace(",", "", $post['bank_out_price']), 'bank_out_balance_after' => str_replace(",", "", $post['bank_out_balance_after']), 'bank_out_remark' => $post['bank_out_remark'], 'bank_out_edit_by' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.bank_out_name = tb_bookbank_1.bank_cus_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_bookbank_1.bank_cus_name AS bankOutNameBankCusName, tb_bookbank_1.bank_name AS bankOutNameBankName, tb_bookbank_1.bank_branch AS bankOutNameBankBranch, tb_bookbank_1.bank_number AS bankOutNameBankNumber
				");
		$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.bank_out_name = tb_bookbank_1.bank_cus_name", 'left');
	...
	{
		$data = array(
			'bank_out_id' => $post['bank_out_id'],
			'bank_out_date' => setDateToStandard($post['bank_out_date']),
			'bank_out_name' => $post['bank_out_name'],
			'bank_out_balance_before' => str_replace(",", "", $post['bank_out_balance_before']),
			'bank_out_price' => str_replace(",", "", $post['bank_out_price']),
			'bank_out_balance_after' => str_replace(",", "", $post['bank_out_balance_after']),
			'bank_out_remark' => $post['bank_out_remark'],
			'bank_out_edit_by' => $this->session->userdata('user_firstname')
		);
