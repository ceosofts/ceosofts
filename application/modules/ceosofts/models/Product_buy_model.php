...
/**
 * Product_buy_model Class
 * @date 2021-10-08
 */
class Product_buy_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['prb_id'] = array('format' => 1, 'type' => 'PRB', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				'prb_id' => $post['prb_id']
				,'prb_name' => $post['prb_name']
				,'prb_price' => str_replace(",", "", $post['prb_price'])
				,'prb_unit' => $post['prb_unit']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_unit AS tb_unit_1', "$this->my_table.prb_unit = tb_unit_1.unit_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_unit_1.unit_name AS prbUnitUnitName
				");
		$this->db->join('tb_unit AS tb_unit_1', "$this->my_table.prb_unit = tb_unit_1.unit_name", 'left');
