...
class Members_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['user_id'] = array('format' => 5, 'type' => 'USE', 'digit' => 4);
	...
	public function exists($data)
	{
		...
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.userid = $userid");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.userid = $id");
	...
	public function create($post)
	{
		...
		$data = array(
			'user_id' => $post['user_id'], 'username' => $post['username'], 'password' => $post['password'], 'referral_code' => $post['referral_code'], 'email' => $post['email'], 'prefix' => $post['prefix'], 'firstname' => $post['firstname'], 'lastname' => $post['lastname'], 'photo' => $post['photo'], 'sex' => $post['sex'], 'tel_number' => $post['tel_number'], 'line_id' => $post['line_id'], 'level' => $post['level'], 'department_id' => $post['department_id'], 'unsubscribe' => $post['unsubscribe'], 'void' => $post['void'], 'create_datetime' => date('Y-m-d'), 'create_user_id'  => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_members_prefix AS tb_members_prefix_1', "$this->my_table.prefix = tb_members_prefix_1.prefix_name", 'left');
			$this->db->join('tb_members_level AS tb_members_level_2', "$this->my_table.level = tb_members_level_2.level_value", 'left');
			$this->db->join('tb_department AS tb_department_3', "$this->my_table.department_id = tb_department_3.dpm_id", 'left');
			$this->db->join('tb_status AS tb_status_4', "$this->my_table.void = tb_status_4.dpm_void", 'left');
		...
		$this->db->select("$this->my_table.*, tb_members_prefix_1.prefix_name AS prefixPrefixName
				, tb_members_level_2.level_title AS levelLevelTitle
				, tb_department_3.dpm_name AS departmentIdDpmName
				, tb_status_4.dpm_name AS voidDpmName
				");
		$this->db->join('tb_members_prefix AS tb_members_prefix_1', "$this->my_table.prefix = tb_members_prefix_1.prefix_name", 'left');
		$this->db->join('tb_members_level AS tb_members_level_2', "$this->my_table.level = tb_members_level_2.level_value", 'left');
		$this->db->join('tb_department AS tb_department_3', "$this->my_table.department_id = tb_department_3.dpm_id", 'left');
	...
	{
		$data = array(
			'user_id' => $post['user_id'], 'username' => $post['username'], 'password' => $post['password'], 'referral_code' => $post['referral_code'], 'email' => $post['email'], 'prefix' => $post['prefix'], 'firstname' => $post['firstname'], 'lastname' => $post['lastname'], 'sex' => $post['sex'], 'tel_number' => $post['tel_number'], 'line_id' => $post['line_id'], 'level' => $post['level'], 'department_id' => $post['department_id'], 'unsubscribe' => $post['unsubscribe'], 'void' => $post['void'], 'modify_datetime' => date('Y-m-d'), 'modify_user_id'  => $this->session->userdata('user_firstname')
		);
