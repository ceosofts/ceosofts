...
class Supplier_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['sup_id'] = array('format' => 1, 'type' => 'SUP', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			...
			'sup_contact' => $post['sup_contact'],
			'sup_address' => $post['sup_address'],
			'sup_tel' => $post['sup_tel'],
			'sup_tax' => $post['sup_tax'],
			'sup_branch' => $post['sup_branch'],
			'sup_by' => $this->session->userdata('user_firstname'),
			'sup_date' => date('Y-m-d')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.sup_branch = tb_branch_1.branch_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_branch_1.branch_name AS supBranchBranchName
				");
		$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.sup_branch = tb_branch_1.branch_name", 'left');
