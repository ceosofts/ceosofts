...
/**
 * Stock_out_model Class
 * @date 2021-10-08
 */
class Stock_out_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['sto_id'] = array('format' => 1, 'type' => 'STO', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				'sto_id' => $post['sto_id']
				,'sto_date' => setDateToStandard($post['sto_date'])
				,'sto_cus' => $post['sto_cus']
				,'sto_project_name' => $post['sto_project_name']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_customer AS tb_customer_1', "$this->my_table.sto_cus = tb_customer_1.cus_name", 'left');
		$this->db->join('tb_quotation AS tb_quotation_2', "$this->my_table.sto_project_name = tb_quotation_2.quo_project_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_customer_1.cus_name AS stoCusCusName, tb_customer_1.cus_contact AS stoCusCusContact, tb_customer_1.cus_address AS stoCusCusAddress, tb_customer_1.cus_tel AS stoCusCusTel, tb_customer_1.cus_tax AS stoCusCusTax, tb_customer_1.cus_branch AS stoCusCusBranch
				, tb_quotation_2.quo_project_name AS stoProjectNameQuoProjectName
				");
		$this->db->join('tb_customer AS tb_customer_1', "$this->my_table.sto_cus = tb_customer_1.cus_name", 'left');
		$this->db->join('tb_quotation AS tb_quotation_2', "$this->my_table.sto_project_name = tb_quotation_2.quo_project_name", 'left');
	...
	{
		$this->set_table_name('tb_stock_out_list');
		$this->db->select("tb_stock_out_list.*, tb_product_sale_1.prs_name AS detailStoNamePrsName");
		$this->db->join('tb_product_sale AS tb_product_sale_1', "tb_stock_out_list.sto_name = tb_product_sale_1.prs_name", 'left');
		$this->set_where("sto_ref = '$master_ref_id'");
	...
	public function save_detail_list($post)
	{
		$this->set_table_name('tb_stock_out_list');
		$data = array(
				'sto_ref' => $post['sto_ref']
				,'sto_id' => $post['sto_id']
				,'sto_name' => $post['sto_name']
				,'sto_unit' => $post['sto_unit']
				,'sto_qty' => str_replace(",", "", $post['sto_qty'])
				,'sto_remark' => $post['sto_remark']
		);
