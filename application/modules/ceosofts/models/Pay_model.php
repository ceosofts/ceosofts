...
class Pay_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['pay_id'] = array('format' => 1, 'type' => 'PAY', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'pay_id' => $post['pay_id'],
			'pay_pr_ref' => $post['pay_pr_ref'],
			'pay_date' => setDateToStandard($post['pay_date']),
			'pay_sup' => $post['pay_sup'],
			'pay_project_name' => $post['pay_project_name'],
			'pay_price' => str_replace(",", "", $post['pay_price']),
			'pay_pay_by' => $post['pay_pay_by'], //จ่ายโดย
			'pay_remark' => $post['pay_remark'], //หมายเหตุ
			'pay_pay_date' => setDateToStandard($post['pay_pay_date']), //วันที่นัดจ่าย
			'pay_by' => $this->session->userdata('user_firstname') //สร้างเอกสารโดย
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_pob AS tb_pob_1', "$this->my_table.pay_pr_ref = tb_pob_1.pob_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_pob_1.pob_id AS payPrRefPobId
				");
		$this->db->join('tb_pob AS tb_pob_1', "$this->my_table.pay_pr_ref = tb_pob_1.pob_id", 'left');
	...
	public function update($post)
	{
		$data = array(
			...
			'pay_project_name' => $post['pay_project_name'],
			'pay_price' => str_replace(",", "", $post['pay_price']),
			'pay_pay_by' => $post['pay_pay_by'],
			'pay_remark' => $post['pay_remark'],
			'pay_pay_date' => setDateToStandard($post['pay_pay_date']), //วันที่นัดจ่าย
			'pay_edit_by' => $this->session->userdata('user_firstname'), //คนที่แก้ไข
			'pay_edit_date' => date('Y-m-d') //วันที่แก้ไข
		);
