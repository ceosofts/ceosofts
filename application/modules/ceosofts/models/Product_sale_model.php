...
class Product_sale_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['prs_id'] = array('format' => 1, 'type' => 'PRS', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				'prs_id' => $post['prs_id']
				,'prs_name' => $post['prs_name']
				,'prs_price' => str_replace(",", "", $post['prs_price'])
				,'prs_unit' => $post['prs_unit']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_unit AS tb_unit_1', "$this->my_table.prs_unit = tb_unit_1.unit_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_unit_1.unit_name AS prsUnitUnitName
				");
		$this->db->join('tb_unit AS tb_unit_1', "$this->my_table.prs_unit = tb_unit_1.unit_name", 'left');
