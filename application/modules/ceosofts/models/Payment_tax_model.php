...
/**
 * Payment_tax_model Class
 * @date 2021-10-08
 */
class Payment_tax_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['pay_tax_id'] = array('format' => 1, 'type' => 'PAT', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'pay_tax_id' => $post['pay_tax_id'],
			'pay_tax_pay_ref' => $post['pay_tax_pay_ref'],
			'pay_tax_date' => setDateToStandard($post['pay_tax_date']),
			'pay_tax_sup' => $post['pay_tax_sup'],
			'pay_tax_project_name' => $post['pay_tax_project_name'],
			'pay_tax_price' => str_replace(",", "", $post['pay_tax_price']),
			'pay_tax_pay_by' => $post['pay_tax_pay_by'],
			'pay_tax_remark' => $post['pay_tax_remark'],
			'pay_tax_scan' => $post['pay_tax_scan'],
			'pay_tax_pay_date' => setDateToStandard($post['pay_tax_pay_date']), //วันที่ได้รับเอกสาร
			'pay_tax_by' => $this->session->userdata('user_firstname') //ผู้ออกเอกสาร
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_pay AS tb_pay_1', "$this->my_table.pay_tax_pay_ref = tb_pay_1.pay_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_pay_1.pay_id AS payTaxPayRefPayId
				");
		$this->db->join('tb_pay AS tb_pay_1', "$this->my_table.pay_tax_pay_ref = tb_pay_1.pay_id", 'left');
	...
	public function update($post)
	{
		$data = array(
			...
			'pay_tax_pay_by' => $post['pay_tax_pay_by'],
			'pay_tax_remark' => $post['pay_tax_remark'],
			'pay_tax_pay_date' => setDateToStandard($post['pay_tax_pay_date']),
			'pay_tax_edit_by' => $this->session->userdata('user_firstname'),
			'pay_tax_edit_date' => date('Y-m-d')
		);

		if (isset($post['pay_tax_scan'])) {
			$data['pay_tax_scan'] = $post['pay_tax_scan'];
		}
