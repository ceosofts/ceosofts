...
/**
 * Cheque_out_model Class
 * @date 2021-10-08
 */
class Cheque_out_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['chequeout_id'] = array('format' => 1, 'type' => 'COU', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'chequeout_id' => $post['chequeout_id'], 'chequeout_make_item' => setDateToStandard($post['chequeout_make_item']), 'chequeout_pay_date' => setDateToStandard($post['chequeout_pay_date']), 'chequeout_plan_date' => setDateToStandard($post['chequeout_plan_date']), 'chequeout_to' => $post['chequeout_to'], 'chequeout_price' => str_replace(",", "", $post['chequeout_price']), 'chequeout_bookbank' => $post['chequeout_bookbank'], 'chequeout_number' => $post['chequeout_number'], 'chequeout_status' => $post['chequeout_status'], 'chequeout_photo' => $post['chequeout_photo'], 'chequeout_remark' => $post['chequeout_remark'], 'chequeout_user_make' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.chequeout_bookbank = tb_bookbank_1.bank_cus_name", 'left');
			$this->db->join('tb_cheque_status AS tb_cheque_status_2', "$this->my_table.chequeout_status = tb_cheque_status_2.chs_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_bookbank_1.bank_cus_name AS chequeoutBookbankBankCusName, tb_bookbank_1.bank_name AS chequeoutBookbankBankName, tb_bookbank_1.bank_branch AS chequeoutBookbankBankBranch, tb_bookbank_1.bank_number AS chequeoutBookbankBankNumber
				, tb_cheque_status_2.chs_name AS chequeoutStatusChsName
				");
		$this->db->join('tb_bookbank AS tb_bookbank_1', "$this->my_table.chequeout_bookbank = tb_bookbank_1.bank_cus_name", 'left');
		$this->db->join('tb_cheque_status AS tb_cheque_status_2', "$this->my_table.chequeout_status = tb_cheque_status_2.chs_name", 'left');
	...
	{
		$data = array(
			'chequeout_id' => $post['chequeout_id'], 'chequeout_make_item' => setDateToStandard($post['chequeout_make_item']), 'chequeout_pay_date' => setDateToStandard($post['chequeout_pay_date']), 'chequeout_plan_date' => setDateToStandard($post['chequeout_plan_date']), 'chequeout_to' => $post['chequeout_to'], 'chequeout_price' => str_replace(",", "", $post['chequeout_price']), 'chequeout_bookbank' => $post['chequeout_bookbank'], 'chequeout_number' => $post['chequeout_number'], 'chequeout_status' => $post['chequeout_status'], 'chequeout_remark' => $post['chequeout_remark'], 'chequeout_user_make' => $this->session->userdata('user_firstname')
		);

		if (isset($post['chequeout_photo'])) {
			$data['chequeout_photo'] = $post['chequeout_photo'];
		}
