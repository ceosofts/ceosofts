...
class Stock_in_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['sti_id'] = array('format' => 1, 'type' => 'STI', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'sti_id' => $post['sti_id'], 'sti_date' => setDateToStandard($post['sti_date']), 'sti_sup' => $post['sti_sup'], 'sti_project_name' => $post['sti_project_name'], 'sti_by' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_supplier AS tb_supplier_1', "$this->my_table.sti_sup = tb_supplier_1.sup_name", 'left');
			$this->db->join('tb_quotation AS tb_quotation_2', "$this->my_table.sti_project_name = tb_quotation_2.quo_project_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_supplier_1.sup_name AS stiSupSupName
				, tb_quotation_2.quo_project_name AS stiProjectNameQuoProjectName
				");
		$this->db->join('tb_supplier AS tb_supplier_1', "$this->my_table.sti_sup = tb_supplier_1.sup_name", 'left');
		$this->db->join('tb_quotation AS tb_quotation_2', "$this->my_table.sti_project_name = tb_quotation_2.quo_project_name", 'left');
	...
	{
		$data = array(
			'sti_id' => $post['sti_id'], 'sti_date' => setDateToStandard($post['sti_date']), 'sti_sup' => $post['sti_sup'], 'sti_project_name' => $post['sti_project_name'], 'sti_by' => $post['sti_by']
		);
	...
	{
		$this->set_table_name('tb_stock_in_list');
		$this->db->select("tb_stock_in_list.*, tb_product_buy_1.prb_name AS detailStiNamePrbName");
		$this->db->join('tb_product_buy AS tb_product_buy_1', "tb_stock_in_list.sti_name = tb_product_buy_1.prb_name", 'left');
		$this->set_where("sti_ref = '$master_ref_id'");
	...
	public function save_detail_list($post)
	{
		$this->set_table_name('tb_stock_in_list');
		$data = array(
			'sti_ref' => $post['sti_ref'], 'sti_id' => $post['sti_id'], 'sti_name' => $post['sti_name'], 'sti_unit' => $post['sti_unit'], 'sti_qty' => str_replace(",", "", $post['sti_qty']), 'sti_remark' => $post['sti_remark']
		);
