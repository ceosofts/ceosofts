...
/**
 * Employees_salary_model Class
 * @date 2021-10-10
 */
class Employees_salary_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_employees_salary';
	...
	public function create($post)
	{
		...
		$data = array(
				'ems_id' => $post['ems_id']
				,'ems_fname' => $post['ems_fname']
				,'ems_lname' => $post['ems_lname']
				,'ems_salary' => str_replace(",", "", $post['ems_salary'])
				,'ems_remark' => $post['ems_remark']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_employees AS tb_employees_1', "$this->my_table.ems_id = tb_employees_1.emp_id", 'left');
		...
		$this->db->select("$this->my_table.*, tb_employees_1.emp_id AS emsIdEmpId
				");
		$this->db->join('tb_employees AS tb_employees_1', "$this->my_table.ems_id = tb_employees_1.emp_id", 'left');
