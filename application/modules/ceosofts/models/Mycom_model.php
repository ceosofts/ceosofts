 ...
 * @date 2021-10-05
 */
class Mycom_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['mycom_id'] = array('format' => 5, 'type' => 'MYC', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
			'mycom_id' => $post['mycom_id'], 'mycom_name' => $post['mycom_name'], 'mycom_contact' => $post['mycom_contact'], 'mycom_address' => $post['mycom_address'], 'mycom_tel' => $post['mycom_tel'], 'mycom_tax' => $post['mycom_tax'], 'mycom_branch' => $post['mycom_branch'], 'mycom_logo' => $post['mycom_logo'], 'mycom_edit_date' => date('Y-m-d')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.mycom_branch = tb_branch_1.branch_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_branch_1.branch_name AS mycomBranchBranchName
				");
		$this->db->join('tb_branch AS tb_branch_1', "$this->my_table.mycom_branch = tb_branch_1.branch_name", 'left');
	...
	{
		$data = array(
			'mycom_id' => $post['mycom_id'], 'mycom_name' => $post['mycom_name'], 'mycom_contact' => $post['mycom_contact'], 'mycom_address' => $post['mycom_address'], 'mycom_tel' => $post['mycom_tel'], 'mycom_tax' => $post['mycom_tax'], 'mycom_branch' => $post['mycom_branch'], 'mycom_edit_date' => date('Y-m-d')
		);

		if (isset($post['mycom_logo'])) {
			$data['mycom_logo'] = $post['mycom_logo'];
		}
