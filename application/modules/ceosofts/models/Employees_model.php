...
class Employees_model extends MY_Model
{
	...
	public function __construct()
	{
		...
		$options = array();
		$options['emp_id'] = array('format' => 5, 'type' => 'EMP', 'digit' => 4);
	...
	public function create($post)
	{
		...
		$data = array(
				'emp_id' => $post['emp_id']
				,'emp_title_name' => $post['emp_title_name']
				,'emp_fname' => $post['emp_fname']
				,'emp_lname' => $post['emp_lname']
				,'emd_id_card' => $post['emd_id_card']
				,'emp_photo' => $post['emp_photo']
				,'emp_sex' => $post['emp_sex']
				,'emp_birthday' => setDateToStandard($post['emp_birthday'])
				,'emp_age' => str_replace(",", "", $post['emp_age'])
				,'emp_position' => $post['emp_position']
				,'emp_section' => $post['emp_section']
				,'emp_tel' => $post['emp_tel']
				,'emp_start' => setDateToStandard($post['emp_start'])
				,'emp_time' => str_replace(",", "", $post['emp_time'])
				,'emp_end' => setDateToStandard($post['emp_end'])
				,'emp_holiday_max' => $post['emp_holiday_max']
				,'emp_holiday_off' => $post['emp_holiday_off']
				,'emp_dayoff_day' => $post['emp_dayoff_day']
				,'emp_dayoff_off' => $post['emp_dayoff_off']
				,'emp_ss' => $post['emp_ss']
				,'emp_welfare' => $post['emp_welfare']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			$this->db->join('tb_members_prefix AS tb_members_prefix_1', "$this->my_table.emp_title_name = tb_members_prefix_1.prefix_name", 'left');
		$this->db->join('tb_position AS tb_position_2', "$this->my_table.emp_position = tb_position_2.position_name", 'left');
		$this->db->join('tb_department AS tb_department_3', "$this->my_table.emp_section = tb_department_3.dpm_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_members_prefix_1.prefix_name AS empTitleNamePrefixName
				, tb_position_2.position_name AS empPositionPositionName
				, tb_department_3.dpm_name AS empSectionDpmName
				");
		$this->db->join('tb_members_prefix AS tb_members_prefix_1', "$this->my_table.emp_title_name = tb_members_prefix_1.prefix_name", 'left');
		$this->db->join('tb_position AS tb_position_2', "$this->my_table.emp_position = tb_position_2.position_name", 'left');
		$this->db->join('tb_department AS tb_department_3', "$this->my_table.emp_section = tb_department_3.dpm_name", 'left');
	...
	public function update($post)
	{
		...
		if(isset($post['emp_photo'])){
			$data['emp_photo'] = $post['emp_photo'];
		}
