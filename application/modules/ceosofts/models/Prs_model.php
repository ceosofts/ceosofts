 ...
 * @date 2021-10-08
 */
class Prs_model extends MY_Model
{
	...
	public function create($post)
	{
		...
		$data = array(
			...
			'pr_by' => $this->session->userdata('user_firstname')
		);
		$this->set_table_name($this->my_table);
	...
	public function read($start_row = FALSE, $per_page = FALSE)
	{
		...
		if ($where != '') {
			...
			$this->db->join('tb_quotation_status AS tb_quotation_status_2', "$this->my_table.pr_status = tb_quotation_status_2.stq_name", 'left');
		...
		$this->db->select("$this->my_table.*, tb_supplier_1.sup_name AS prSupSupName, tb_supplier_1.sup_contact AS prSupSupContact, tb_supplier_1.sup_address AS prSupSupAddress, tb_supplier_1.sup_tax AS prSupSupTax, tb_supplier_1.sup_branch AS prSupSupBranch
				, tb_quotation_status_2.stq_name AS prStatusStqName
				");
		$this->db->join('tb_supplier AS tb_supplier_1', "$this->my_table.pr_sup = tb_supplier_1.sup_name", 'left');
		$this->db->join('tb_quotation_status AS tb_quotation_status_2', "$this->my_table.pr_status = tb_quotation_status_2.stq_name", 'left');
	...
	public function update($post)
	{
		$data = array(
			...
			'pr_project_name' => $post['pr_project_name'],
			'pr_status' => $post['pr_status'],
			'pr_price' => str_replace(",", "", $post['pr_price']),
			// 'pr_by' => $post['pr_by'],
			'pr_edit_by' => $this->session->userdata('user_firstname'),
	...
	{
		$this->set_table_name('tb_prs_list');
		$this->db->select("tb_prs_list.*, tb_product_buy_1.prb_name AS detailPrNamePrbName");
		$this->db->join('tb_product_buy AS tb_product_buy_1', "tb_prs_list.pr_name = tb_product_buy_1.prb_name", 'left');
