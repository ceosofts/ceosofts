...
/**
 * Status_stock_qty_model Class
 * @date 2021-10-05
 */
class Status_stock_qty_model extends MY_Model
{
	...
	{
		parent::__construct();
		$this->my_table = 'tb_status_stock_qty';
	...
	{
		$dpm_id = checkEncryptData($data['dpm_id']);
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.dpm_id = $dpm_id");
	...
	{
		$this->set_table_name($this->my_table);
		$this->set_where("$this->my_table.dpm_id = $id");
	...
	public function create($post)
	{
		...
		$data = array(
				'stc_name' => $post['stc_name']
				,'stc_void' => $post['stc_void']
		);
		$this->set_table_name($this->my_table);
	...
	*/
	...
	public function update($post)
	{
		...
		if(!empty($data)){
			$dpm_id = checkEncryptData($post['encrypt_dpm_id']);
			$this->set_table_name($this->my_table);
			$this->set_where("$this->my_table.dpm_id = $dpm_id");
