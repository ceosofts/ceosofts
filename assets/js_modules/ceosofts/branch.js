...
var Branch = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/branch/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/branch/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/branch');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/branch/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/branch/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Branch.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSaveEdit').click(function() {
		return Branch.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Branch.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Branch.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Branch.confirmDelete(pId,  row_num);
	});//click
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/branch/' + $(this).attr('onclick-action'));
