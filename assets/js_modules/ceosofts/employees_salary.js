...
var EmployeesSalary = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/employees_salary/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/employees_salary/save');
		var obj = $('#btnConfirmSave');
		if(loading_on(obj) == true){


			var ems_salary = removeComma($('#ems_salary').val());
			$('#ems_salary').val(ems_salary);
			...
			$.ajax({
				...
				success: function (results) {
					...
					var ems_salary = formatNumber($('#ems_salary').val(), 2);
					$('#ems_salary').val(ems_salary);
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/employees_salary');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/employees_salary/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/employees_salary/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(EmployeesSalary.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
	...
	readExcel: function(){
		var frm_action = site_url('ceosofts/employees_salary/read_excel');
	...
	saveExcel: function(){
		var frm_action = site_url('ceosofts/employees_salary/save_excel_data');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						alert_type = 'success';
						goto_page('ceosofts/employees_salary');
					}else{
						alert_type = 'danger';
					}
					notify('บันทึกข้อมูล', results.message, alert_type, 'center');
...
$(document).ready(function() {
	...
	$(document).on('click', '#btnSaveExcel', function() {
		return EmployeesSalary.validateSaveExcel();
	});
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		EmployeesSalary.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return EmployeesSalary.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		EmployeesSalary.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		EmployeesSalary.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		EmployeesSalary.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		EmployeesSalary.deleteRecord();
	});
	setDropdownList('#ems_id');
