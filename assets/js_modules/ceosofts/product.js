...
var Product = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/product/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/product/save');
		var obj = $('#btnConfirmSave');
		if(loading_on(obj) == true){


			var pro_price = removeComma($('#pro_price').val());
			...
			$.ajax({
				...
				success: function (results) {
					...
					var pro_price = formatNumber($('#pro_price').val(), 2);
					$('#pro_price').val(pro_price);
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/product');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/product/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/product/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Product.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Product.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Product.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Product.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Product.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Product.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Product.deleteRecord();
	});
	setDropdownList('#pro_unit');
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/product/' + $(this).attr('onclick-action'));
