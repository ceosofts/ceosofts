...
var Bookbank = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/bookbank/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/bookbank/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/bookbank');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/bookbank/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/bookbank/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Bookbank.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Bookbank.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Bookbank.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Bookbank.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Bookbank.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Bookbank.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Bookbank.deleteRecord();
	});
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/bookbank/' + $(this).attr('onclick-action'));
