...
var Members = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/members/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/members/save');
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/members/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/members/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Members.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	if(typeof param_current_path != 'undefined'){
		Members.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/members/' + $(this).attr('onclick-action'));
