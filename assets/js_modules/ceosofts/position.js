...
var Position = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/position/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/position/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/position');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/position/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/position/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Position.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Position.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Position.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Position.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Position.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Position.confirmDelete(pId,  row_num);
	});//click
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/position/' + $(this).attr('onclick-action'));
