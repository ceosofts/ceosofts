...
var Unit = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/unit/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/unit/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/unit');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/unit/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/unit/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Unit.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	if(typeof param_current_page != 'undefined'){
		Unit.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Unit.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Unit.confirmDelete(pId,  row_num);
	});//click
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/unit/' + $(this).attr('onclick-action'));
