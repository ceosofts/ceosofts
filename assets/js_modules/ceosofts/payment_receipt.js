...
var PaymentReceipt = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/payment_receipt/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/payment_receipt/save');
		...
		if(loading_on(obj) == true){
			...
			var pay_rec_price = removeComma($('#pay_rec_price').val());
			$('#pay_rec_price').val(pay_rec_price);
			...
			$("#post_iframe").on('load',function() {
				...
				if(c==1){
					...
					var pay_rec_price = formatNumber($('#pay_rec_price').val(), 2);
					$('#pay_rec_price').val(pay_rec_price);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/payment_receipt/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/payment_receipt/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(PaymentReceipt.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		PaymentReceipt.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return PaymentReceipt.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		PaymentReceipt.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		PaymentReceipt.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		PaymentReceipt.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		PaymentReceipt.deleteRecord();
	});
	setDropdownList('#pay_rec_pr_ref');
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/payment_receipt/' + $(this).attr('onclick-action'));
