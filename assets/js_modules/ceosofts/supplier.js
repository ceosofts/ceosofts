...
var Supplier = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/supplier/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/supplier/save');
		...
		if(loading_on(obj) == true){
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/supplier');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/supplier/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/supplier/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Supplier.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Supplier.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Supplier.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Supplier.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Supplier.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Supplier.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Supplier.deleteRecord();
	});
	setDropdownList('#sup_branch');
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/supplier/' + $(this).attr('onclick-action'));
