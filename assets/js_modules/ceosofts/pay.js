...
var Pay = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/pay/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/pay/save');
		var obj = $('#btnConfirmSave');
		if(loading_on(obj) == true){


			var pay_price = removeComma($('#pay_price').val());
			...
			$.ajax({
				...
				success: function (results) {
					...
					var pay_price = formatNumber($('#pay_price').val(), 2);
					$('#pay_price').val(pay_price);
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/pay');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/pay/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/pay/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Pay.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	if(typeof param_current_page != 'undefined'){
		Pay.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Pay.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Pay.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Pay.deleteRecord();
	});
	setDropdownList('#pay_pr_ref');
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/pay/' + $(this).attr('onclick-action'));
