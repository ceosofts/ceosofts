...
var Receipt = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/receipt/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/receipt/save');
		var obj = $('#btnConfirmSave');
		if(loading_on(obj) == true){


			var rec_price = removeComma($('#rec_price').val());
			$('#rec_price').val(rec_price);
			...
			$.ajax({
				...
				success: function (results) {
					...
					var rec_price = formatNumber($('#rec_price').val(), 2);
					$('#rec_price').val(rec_price);
					...
					if(results.is_successful){
						window.location = site_url('ceosofts/receipt');
					}
				},
				error : function(jqXHR, exception){
					ajaxErrorMessage(jqXHR, exception);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/receipt/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/receipt/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Receipt.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
	...
	readExcel: function(){
		var frm_action = site_url('ceosofts/receipt/read_excel');
	...
	saveExcel: function(){
		var frm_action = site_url('ceosofts/receipt/save_excel_data');
	...
	readExcelDetail: function(){
		var frm_action = site_url('ceosofts/receipt/read_excel_detail');
	...
	saveExcelDetail: function(){
		var frm_action = site_url('ceosofts/receipt/save_excel_data_detail');
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						...
						Receipt.loadDetailList($('[name="encrypt_"]').val());
					}else{
						alert_type = 'danger';
					}
					notify('บันทึกข้อมูล', results.message, alert_type, 'center');
...
$(document).ready(function() {
	...
	$(document).on('change','#rec_inv_number_ref', function(){
		var opt = $(this).find('option[value="'+ $(this).val() +'"]');

		var change_val = opt.attr('data-inv_cus');
		$('#rec_cus').val(change_val).attr('value', change_val);
		setDropdownList('#rec_cus');

		var change_val = opt.attr('data-inv_project_name');
		$('#rec_project_name').val(change_val).attr('value', change_val);

		var change_val = opt.attr('data-inv_price_this_period');
		$('#rec_price').val(change_val).attr('value', change_val);
	...
	$(document).on('click', '#btnSaveExcel', function() {
		return Receipt.validateSaveExcel();
	});
		
	$(document).on('click', '#btnReadExcelDetail', function() {
		Receipt.readExcelDetail();
	});
	...
	$(document).on('click', '#btnSaveExcelDetail', function() {
		return Receipt.validateSaveExcelDetail();
	});
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		Receipt.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return Receipt.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		Receipt.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Receipt.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		Receipt.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		Receipt.deleteRecord();
	});
	setDropdownList('#rec_inv_number_ref');
	setDropdownList('#rec_cus');
	setDropdownList('#rec_status');
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/receipt/' + $(this).attr('onclick-action'));
