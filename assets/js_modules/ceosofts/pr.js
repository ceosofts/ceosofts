...
var Pr = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/pr/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/pr/save');
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						...
						$('#btnGotoEdit').attr('href', site_url('ceosofts/pr/edit/'+ encodeURIComponent(results.encrypt_id)));
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/pr/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/pr/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(Pr.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
	...
	openAddListDialog: function(){
		$('#formAddList').attr('action', site_url('ceosofts/pr/save_detail_list'));
		Pr.resetAddFormList();
			...
			$.ajax({
				...
				success: function (results) {
					...
					if(results.is_successful){
						...
						Pr.loadDetailList($('[name="encrypt_id"]').val());
	...
	deleteRecordList: function(){
		var frm_action = site_url('ceosofts/pr/del_list');
	...
	editDetailRecord: function (pId,  irow, url_encrypt_id){
		...
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/pr/edit_list/'+ url_encrypt_id),
			success: function (json_string) {
				try
				{
					...
					if(results.is_successful){
						...
						$('#addListModal').modal('show');
						$('#formAddList').attr('action', site_url('ceosofts/pr/update_list'));
	...
	loadDetailList: function(ref_encrypt_id){
		$.ajax({
			method: 'GET',
			dataType: 'json',
			url: site_url('ceosofts/pr/load_detail/'+ ref_encrypt_id),
			success: function (results) {
...
$(document).ready(function() {
	...
	if(typeof param_current_page != 'undefined'){
		Pr.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		Pr.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-list-row', function(){
		...
		Pr.confirmDeleteList(pId,  row_num);
	});//click
	...
	$(document).on('click','.btn-preview-list-row', function(){
		...
		Pr.previewDetailRecord(pId,  row_num);
	});
	...
	$(document).on('click','.btn-edit-list-row', function(){
		...
		Pr.editDetailRecord(pId,  row_num, url_encrypt_id);
	});

	$(document).on('change','#detail_pr_name',function(){
		$('#detail_pr_id').val($(this).find('option[value="'+$(this).val()+'"]').data('pr_id'));
		$('#detail_pr_price').val($(this).find('option[value="'+$(this).val()+'"]').data('pr_price'));
		$('#detail_pr_unit').val($(this).find('option[value="'+$(this).val()+'"]').data('pr_unit'));
	});
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/pr/' + $(this).attr('onclick-action'));
