...
var PaymentTax = {
	...
	loadPreview: function(id){
		$.ajax({
			method: 'GET',
			url: site_url('ceosofts/payment_tax/preview/'+ id),
	...
	saveFormData: function(){
		var frm_action = site_url('ceosofts/payment_tax/save');
		...
		if(loading_on(obj) == true){
			...
			var pay_tax_price = removeComma($('#pay_tax_price').val());
			$('#pay_tax_price').val(pay_tax_price);
			...
			$("#post_iframe").on('load',function() {
				...
				if(c==1){
					...
					var pay_tax_price = formatNumber($('#pay_tax_price').val(), 2);
					$('#pay_tax_price').val(pay_tax_price);
	...
	saveEditForm: function(){
		...
		var frm_action = site_url('ceosofts/payment_tax/update');
	...
	deleteRecord: function(){
		var frm_action = site_url('ceosofts/payment_tax/del');
		...
		$.ajax({
			...
			success: function (results) {
				if(results.is_successful){
					alert_type = 'success';
					setTimeout(function(){
						$(window.location).attr('href', site_url(PaymentTax.current_path));
					}, 500);
				}else{
					alert_type = 'danger';
				}
...
$(document).ready(function() {
	...
	$('#btnSave').click(function() {
		$('#addModal').modal('hide');
		PaymentTax.saveFormData();
		return false;
	});//click

	$('#btnSaveEdit').click(function() {
		return PaymentTax.validateFormEdit();
	});//click
	...
	if(typeof param_current_page != 'undefined'){
		PaymentTax.current_page = Math.abs(param_current_page);
	}

	if(typeof param_current_path != 'undefined'){
		PaymentTax.current_path = param_current_path;
	}
	...
	$(document).on('click','.btn-delete-row', function(){
		...
		PaymentTax.confirmDelete(pId,  row_num);
	});//click

	$(document).on('click','#btn_confirm_delete', function(){
		PaymentTax.deleteRecord();
	});
	setDropdownList('#pay_tax_pay_ref');
	...
	$(document).on('click','.btn[onclick-action]', function(){
		var action_url = site_url('ceosofts/payment_tax/' + $(this).attr('onclick-action'));
